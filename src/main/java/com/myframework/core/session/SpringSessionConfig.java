...
public class SpringSessionConfig {
    ...
    public JedisConnectionFactory connectionFactory() {
        if (StringUtil.toBoolean(PropertiesUtil.getInstance(Constants.MY_FRAMEWORK_FILE_PATH).getValue(SpringSessionConfig.SESSION_REDIS_ENABLE))) {
            ...
            connection.setPort(Integer.valueOf(PropertiesUtil.getInstance(Constants.MY_FRAMEWORK_FILE_PATH).getValue(SESSION_REDIS_PORT)));
            connection.setHostName(PropertiesUtil.getInstance(Constants.MY_FRAMEWORK_FILE_PATH).getValue(SESSION_REDIS_HOST));
            return connection;
        } else {
            return null;
        }
