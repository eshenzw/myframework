...
public class DeviceUtil {
    ...
    public static boolean isAndroidDevice(HttpServletRequest request) {
        ...
        final String[] android_sys = {"Android", "android"};
        String userAgent = request.getHeader("user-agent");
        for (int i = 0; !isAndroid && userAgent != null && !userAgent.trim().equals("") && i < android_sys.length; i++) {
            if (userAgent.contains(android_sys[i])) {
                isAndroid = true;
                break;
            }
        }
