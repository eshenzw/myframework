...
public class KafkaConsumerAtMostOnceThread implements Runnable
{
	private static Logger logger = LoggerFactory.getLogger(KafkaConsumerAtMostOnceThread.class);
	...
	public void run()
	{
		try
		{
			while (KafkaMessageCenter.keepRunning)
			{
				...
				for (ConsumerRecord<String, String> record : records)
				{
					...
					{
						logger.error("0消息处理失败:{},{}", new Object[]
						{
							key, value
						});
						logger.error("0消息处理失败:", e);
						try
						{
							MessageCenterHandler.sendMessageByTopic(MessageCenterHandler.TOPIC_TODO,
									String.format("%s:%s", MessageCenterHandler.TOPIC_BLOG, key), value);
		...
		{
			logger.error(String.format("0消费者线程运行异常，原因: %s", e.getMessage()), e);
		}
		finally
		{
			logger.warn("0消费者异常结束！");
		}
	}
